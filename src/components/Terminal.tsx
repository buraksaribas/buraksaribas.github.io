...
function Terminal() {
  ...
  const [history, setHistory] = useState<HistoryEntry[]>([
    { command: 'neofetch', output: 'Portfolio OS v1.0\nReactJS 19.1.0 Terminal\nFull Stack Developer & Software Engineer' },
  ]);
...
`,
    ...
    coffee: () => {
      const coffeeTypes = ['Espresso', 'Latte', 'Cappuccino', 'Americano', 'Turkish Coffee'];
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      if (trimmedInput === 'clear') {
        ...
        output = typeof command === 'function' ? command() : command || `Command '${trimmedInput}' not found. Type 'help'`;
  ...
  return (
    <div
      ref={terminalRef}
      className="h-full overflow-y-auto font-mono bg-black text-cyan-300 p-4"
    ...
    >
      {history.map((entry, i) => (
        <div key={i} className="mb-2">
          ...
          {entry.output && (
            <pre className="pl-6 text-cyan-200 text-sm whitespace-pre-wrap">{entry.output}</pre>
          )}
        </div>
      ))}
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          ref={inputRef}
          type="text"
          className="bg-transparent text-cyan-100 outline-none flex-1 font-mono caret-cyan-300"
          value={input}
